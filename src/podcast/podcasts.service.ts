...
import {
    ...
    GetEpisodeOutput,
    GetPodcastsPagenationOutput,
    GetPodcastsCategoryPagenationInput,
...
export class PodcastsService {
    ...
    async getPodcastsPagenation({
        page,
    }): Promise<GetPodcastsPagenationOutput> {
        try {
            const [
                podcasts,
                totalCount,
    ...
    async getPodcastsCategoryPagenation({
        page,
        selectCategory,
    }: GetPodcastsCategoryPagenationInput): Promise<
        GetPodcastsPagenationOutput
    > {
        try {
            const [
    ...
    async createPodcast(
        creator: User,
        { title, category, description, coverImg }: CreatePodcastInput
    ...
    async getPodcast(id: number): Promise<PodcastOutput> {
        try {
            const podcast = await this.podcastRepository.findOne(
                { id },
                { relations: ["episodes", "creator", "reviews", "listeners"] }
            );
            if (!podcast) {
                return {
                    ok: false,
    ...
    async createEpisode(
        user: User,
        { podcastId, title, audioUrl }: CreateEpisodeInput
    ...
    async createReview(
        creator: User,
        { text, podcastId }: CreateReviewInput
        ...
        try {
            ...
            const review = this.reviewRepository.create({ text });
    ...
    async getHostPodcasts(user: User): Promise<GetAllPodcastsOutput> {
        try {
            const { ok, error, podcasts } = await this.getAllPodcasts();
            if (!ok) {
                return { ok, error };
            }
            const hostPodcasts = podcasts.filter(
                (podcast) => podcast.creatorId === user.id
            );
            return {
                ok: true,
                podcasts: hostPodcasts,
            };
        } catch (e) {
            return this.InternalServerErrorOutput;
        }
